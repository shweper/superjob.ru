...
# СЧИТЫВАЕМ ГОРОДА иЗ ЭКСЕЛЯ
wb = xlrd.open_workbook('./DATA.xlsx')
sheet_xlr = wb.sheet_by_name('Города_адреса')
a = 2
...
#БЕРЁМ РАНДОМНУЮ СТРОКУ ГОРОДА
i = random.randint(3, rows_sheet)
################### заводим содержимое в переменные ################################
# print(a)
gorod = (sheet.cell(row=i, column=1).value)
adr = (sheet.cell(row=i, column=2).value)
metro = (sheet.cell(row=i, column=3).value)
'''
barnaul_adr = []
volgograd_adr = []
voronej_adr = []
...
#    print('random adres = ', random_adres)
    ...
    adr_bar.send_keys(adr)
    time.sleep(2)
    adr_bar.send_keys(Keys.ARROW_DOWN + Keys.ENTER)

    ############## Пробуем ввести Метро ###################################

    if metro != None:
        time.sleep(1)
        browser.find_element_by_xpath('/html/body/div[3]/div/div[1]/div[4]/div/div/div/div/div/form/div/div[1]/div/div/div[5]/div/div/div/button').click()
        time.sleep(2)
        browser.find_element_by_xpath('/html/body/div[3]/div/div[5]/div[2]/div/div[2]/div/div/div[2]/div[1]/label/div/div/input').send_keys(metro)
        time.sleep(1)
        browser.find_element_by_xpath('/html/body/div[3]/div/div[5]/div[2]/div/div[2]/div/div/div[2]/div[3]/div/label/div/div[1]/span').click()
